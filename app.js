...
const inventors = [
  ...
  const inventorNames =
  inventorsBornIn1500s.map(inventor => {
    return {
  ...
  const countInstances = data.reduce ((count, item) => {
  ...
  const person19OrOlder = devs.some(dev => currentYear - dev.year >= 19);
  console.log(person19OrOlder);
  
  // Array.prototype.every()
  // 9. Check if everyone is 19 or older?
  const thisYear = new Date().getFullYear();
  const isEveryone19OrOlder = devs.every(dev => thisYear - dev.year >= 19);
